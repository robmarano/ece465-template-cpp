...
static zk::buffer buffer_from(std::string_view str)
{
    return zk::buffer(str.data(), str.data() + str.size());
}
...
int main()
{
    ...
    cout << "Welcome ...";
    auto client = zk::client::connect("zk://10.0.2.2:2181").get();
    ...
    //    client.get_children("/foo").then(print_thing<zk::get_children_result>);

    // set_result has a zk::stat for the modified ZNode.
    //    client.set("/foo/bar", buffer_from("some data")).then(print_thing<zk::set_result>);

    // More explicit: client.create("/foo/baz", "more data", zk::acls::open_unsafe(), zk::create_mode::normal);
    //    client.create("/foo/baz", buffer_from("more data")).then(print_thing<zk::create_result>);
    ...
    //             // zk::op::create("/foo/bap", buffer_from("hi"), nullopt, zk::create_mode::sequential),
    //             zk::op::create("/foo/bap", buffer_from("hi"), zk::create_mode::sequential),
    //             zk::op::erase("/foo/bzr"),
    //         };

    //         // multi_res's type is zk::future<zk::multi_result>
    //         ((zk::client)client).commit(txn).then(print_thing<zk::multi_result>); });
